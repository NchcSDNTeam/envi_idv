...
import org.openflow.gui.net.protocol.LinkSpecsList;
...
public class ConnectionHandler implements MessageProcessor<OFGMessage>,
    ...
    //    FlowTable.setTopology(topology);
    }
    ...
    protected Node processNodeAdd(org.openflow.gui.net.protocol.Node n) {
        if(n.nodeType == NodeType.HOST)
        {
        	...
        	if( !n.ipPortData.equals("nonIP") && !n.ipPortData.equals("0.0.0.0") && !n.ipPortData.equals("CannotGetIp"))
        			return new Host(n.ipPortData,n.id);
        	else
        		return null;
        }
        else
        {   
        	if( topology.getNode(n.id)!= null)
        	{
        		if(topology.getNode(n.id).getType() == NodeType.HOST)
        			topology.removeNode(connection, n.id);
        	}
            return new OpenFlowSwitch(n.id, n.nodeType, n.ipPortData);
        }
    }
    //end
    ...
    */
    ...
    private void processLinksAdd(LinksAdd msg) {
        for(org.openflow.gui.net.protocol.LinkSpec x : msg.links) {
        	if( x!= null && !(x.interDomainLink))
        	{
        		NodeWithPorts dst = topology.getNode(x.dstNode.id);
        		if(dst == null) {
        			System.out.print(x.linkType);
            ...
            if(src.getType() != NodeType.HOST && dst.getType() != NodeType.HOST)
            {
            	if(LinkSpecsList.switchHostLink == 1)
            		continue;
            }
    ...
    private void processFlowsAdd(FlowsAdd msg) {
        for(org.openflow.gui.net.protocol.Flow x : msg.flows) {
        	if( x != null)
        	{
        		...
        		if(x.getFlowDataClass().getEthernetType().equals("ARP"))
        		{//x.getFlowDataClass().setEthernetType("IP");
        			continue;
        		}
        	/*============================avoid to add flow which src host and dst host are island node.========================*/
        		/*
        		String idString = x.getFlowDataClass().getSrcMacAddress();
        		String [] idStringArray = idString.split(":");
        		idString = idStringArray[0]+idStringArray[1]+idStringArray[2]+idStringArray[3]+idStringArray[4]+idStringArray[5];
        		Long hostId = Long.parseLong(idString,16);
        		NodeWithPorts hostNode = topology.getNode(hostId);
        		Collection<Link> srcHostlink = null;
        		if(hostNode!=null)
        			srcHostlink = hostNode.getLinks();
        		else
        			continue; 		
        		idString = x.getFlowDataClass().getDstMacAddress();
        		idStringArray = idString.split(":");
        		idString = idStringArray[0]+idStringArray[1]+idStringArray[2]+idStringArray[3]+idStringArray[4]+idStringArray[5];
        		hostId = Long.parseLong(idString,16);
        		hostNode = topology.getNode(hostId);
        		Collection<Link> dstHostlink = null;
        		if(hostNode != null)
        			dstHostlink = hostNode.getLinks();
        		else
        			continue;
        		if( srcHostlink.isEmpty() && dstHostlink.isEmpty())
        		{
        			System.out.println("avoid to add flow which src host and dst host are island node.");
        			continue;
        		}
        		*/
        		/*=======================================================================================================*/
        		...
        		/*NodeWithPorts middleSrc = null;
        		NodeWithPorts middleDst = null;
        		short middleSrcPort = 0;
        		short middleDstPort = 0;
        		boolean reverse = false;
        		for(FlowData f : FlowData.flowEntries)
        		{
        			reverse = false;
            	
        			FlowData current = x.getFlowDataClass();
        			if( (current.getFid()) != (f.getFid()) )
        			{
        				if( current.getSrcId() != f.getSrcId() && current.getDstId() != f.getDstId())
        				{
        					if( current.getSrcMacAddress().equals(f.getSrcMacAddress()) &&
            					current.getDstMacAddress().equals(f.getDstMacAddress()))
        					{
        						if( current.getEthernetType().equals(f.getEthernetType()))
        						{          					
        							Collection<Link> cl = src.getLinks();
        							for(Link l : cl)
        							{
        								if( src.getID() == (l.getDestination()).getID())
        								{
        									if( (l.getSource()).getID() == f.getDstId())
        									{
            								
        										middleSrc = l.getSource();
        										middleSrcPort = l.getMyPort(middleSrc);
        										middleDst = l.getDestination();
        										middleDstPort = l.getMyPort(middleDst);
        										reverse = true;
        										break;
        									}
        								}
        							...
        							if(!reverse)
        							{
        								...
        								for(Link l : cl)
        								{
        									if( dst.getID() == (l.getSource()).getID())
        									{
        										if( (l.getDestination()).getID() == f.getSrcId())
        										{
        							...
        							if( middleSrc != null && middleDst != null )
        							{
        								FlowHop[] middleHops = new FlowHop[2];
        								middleHops[0] = new FlowHop((short)-1, middleSrc, middleSrcPort);
        								middleHops[middleHops.length-1] = new FlowHop(middleDstPort, middleDst, (short)-1);
        								Flow middleFlow = new Flow(x.type, x.id, middleHops);
        								FlowData newFlowData = new FlowData(middleSrc.getID(),middleDst.getID(),middleSrcPort,middleDstPort,
                							current.getSrcMacAddress(), current.getSrcClientIp(),
                							current.getDstMacAddress(), current.getDstClientIp(),
                							current.getEthernetType(),FlowData.fidCount++);
                					//current.setSrcId(middleSrc.getID());
                					//current.setSrcPort(middleSrcPort);
                					//current.setDstId(middleDst.getID());
                					//current.setDstPort(middleDstPort);
        								if(!newFlowData.repeatFlowEntries())
        									FlowData.flowEntries.add(newFlowData);
                		          //  FlowData.flowEntries.add(newFlowData);
        								topology.addFlow(middleFlow);
