...
public class Link extends AbstractDrawable implements Edge<NodeWithPorts> {
    ...
    private void drawWiredLinkNoPrep(Graphics2D gfx) {
       ...
       if(src.getType() == NodeType.HOST && dst.getType() != NodeType.HOST)
       {
    	   ...
    	   Host hostSrc = (Host)src;
    	   hostSrc.showNode = true;
    	   hostSrc.showInformation = true;
    	   ShapeIcon swIcon = (ShapeIcon) dst.getIcon();
    	   ShapeIcon htIcon = (ShapeIcon) src.getIcon();
    	   htIcon.setFillColor(swIcon.getFillColor());

    	   
    	   
       }
       if(dst.getType() == NodeType.HOST && src.getType() != NodeType.HOST)
       {    	   
    	   ShapeIcon htIcon = (ShapeIcon) dst.getIcon();
    	   ShapeIcon swIcon = (ShapeIcon) src.getIcon();
    	   htIcon.setFillColor(swIcon.getFillColor());
       }
