...
public abstract class LinkSpecsList extends OFGMessage {
    ...
    public static int switchHostLink = 0;
    ...
    public LinkSpecsList(final int len, final OFGMessageType t, final int xid, final DataInput in) throws IOException {
        ...
        while(left >= LinkSpec.SIZEOF) {
            ...
            switchHostLink = in.readShort();
            int interDomainLink = in.readShort();
            if(switchHostLink == 0)
            {
            ...
            {
            	if(interDomainLink == 0)
            	{
            		links[index++] = new LinkSpec(in,false);
            	}
            	else
            	{
            		LinkData.interDomainLinkList.add(new LinkSpec(in,true));
            	//	links[index++] = new LinkSpec(in,true);
            	}
            }
        }
    }
